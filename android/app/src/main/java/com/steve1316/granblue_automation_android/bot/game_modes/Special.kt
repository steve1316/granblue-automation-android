...
class Special(private val game: Game, private val mapName: String, private val missionName: String) {
	...
	private class SpecialException(message: String) : Exception(message)
	...
	fun checkDimensionalHalo(): Boolean {
		if (game.configData.enableNightmare && game.imageUtils.confirmLocation("limited_time_quests", tries = 3)) {
			MessageLog.printToLog("\n[D.HALO] Detected Dimensional Halo. Starting it now...", tag)

			MessageLog.printToLog("\n********************", tag)
			MessageLog.printToLog("********************", tag)
			MessageLog.printToLog("[D.HALO] Dimensional Halo", tag)
			MessageLog.printToLog("[D.HALO] Dimensional Halo Summons: ${game.configData.nightmareSummons}", tag)
			MessageLog.printToLog("[D.HALO] Dimensional Halo Group Number: ${game.configData.nightmareGroupNumber}", tag)
			MessageLog.printToLog("[D.HALO] Dimensional Halo Party Number: ${game.configData.nightmarePartyNumber}", tag)
			MessageLog.printToLog("********************", tag)
			MessageLog.printToLog("\n********************", tag)
			...
			MessageLog.printToLog("\n[D.HALO] Dimensional Halo detected but user opted to not run it. Moving on...", tag)
			game.findAndClickButton("close")
		} else {
			MessageLog.printToLog("\n[D.HALO] No Dimensional Halo detected. Moving on...", tag)
		}
			...
			if (game.imageUtils.confirmLocation("special")) {
				...
				while (tries > 0) {
					...
					if (missionLocation != null) {
						...
						MessageLog.printToLog("[SPECIAL] Navigating to $mapName...", tag)
						...
						when (mapName) {
							"Uncap Treasure Quests" -> {
								...
								MessageLog.printToLog("[SPECIAL] Now selecting $difficulty difficulty...", tag)
							...
							"Shiny Slime Search!", "Angel Halo" -> {
								// Open up the mission's difficulty selection popup and then select its difficulty.
								MessageLog.printToLog("[SPECIAL] Now selecting $difficulty $mapName...", tag)
							...
							"Showdowns" -> {
								MessageLog.printToLog("[SPECIAL] Opening up $formattedMissionName mission popup...", tag)
								var roundPlayButtonLocations: ArrayList<Point> = game.imageUtils.findAll("play_round_button")
							...
							"Campaign-Exclusive Quest" -> {
								MessageLog.printToLog("[SPECIAL] Selecting Campaign-Exclusive Quest...", tag)
