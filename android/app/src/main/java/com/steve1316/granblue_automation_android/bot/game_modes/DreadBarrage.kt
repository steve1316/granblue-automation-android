...
class DreadBarrage(private val game: Game, private val missionName: String) {
	...
	private class DreadBarrageException(message: String) : Exception(message)
	...
	private fun navigate() {
		...
		MessageLog.printToLog("\n[DREAD.BARRAGE] Now navigating to Dread Barrage...", tag)
		...
		if (game.imageUtils.confirmLocation("dread_barrage")) {
			...
			if (game.imageUtils.confirmLocation("resume_quests")) {
				MessageLog.printToLog("\n[WARNING] Detected that there is already a hosted Dread Barrage mission.", tag)
				...
				while (game.imageUtils.confirmLocation("resume_quests", tries = 1)) {
					...
					MessageLog.printToLog("\n[WARNING] The bot will now either wait for the expiry time of 1 hour and 30 minutes or for someone else in the room to clear it.", tag)
					MessageLog.printToLog("[WARNING] The bot will now refresh the page every 30 seconds to check if it is still there before proceeding.", tag)
					MessageLog.printToLog("User can either wait it out, revive and fight it to completion, or retreat from the mission manually.", tag)
				...
				MessageLog.printToLog("\n[SUCCESS] Hosted Dread Barrage mission is now gone either because of timeout or someone else in the room killed it. Moving on...", tag)
			}
